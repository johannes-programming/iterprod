...
jacobus: works
	...
	conda run -n works python -m jacobus @make/jacobus_empty_1.txt;
...
test:
	conda run -n base python make/env.py test_iterprod --python=3.11 --recreate >/dev/null;
	conda run -n test_iterprod pip install -e . >/dev/null;
	conda run -n test_iterprod python -c 'import iterprod.tests; iterprod.tests.test();';
	conda run -n test_iterprod pip install mypy >/dev/null;
	conda run -n test_iterprod python -m mypy --strict .;
	conda run -n test_iterprod python -m mypy --strict -p iterprod;
